// Class definition
var KTProductify = function() {

    // Private functions
    var demo1 = function() {
        var input = document.getElementById('kt_Productify_1'),
            // init Productify script on the above inputs
            Productify = new Productify(input, {
        ...
        // "remove all Products" button event listener
        document.getElementById('kt_Productify_1_remove').addEventListener('click', Productify.removeAllProducts.bind(Productify))

        // Chainable event listeners
        Productify.on('add', onAddProduct)
            .on('remove', onRemoveProduct)
            .on('input', onInput)
            .on('edit', onProductEdit)
            .on('invalid', onInvalidProduct)
        ...
        function onAddProduct(e) {
            console.log("onAddProduct: ", e.detail);
            console.log("original input value: ", input.value)
            Productify.off('add', onAddProduct) // exmaple of removing a custom Productify event
        }

        // Product remvoed callback
        function onRemoveProduct(e) {
            console.log(e.detail);
            console.log("Productify instance value:", Productify.value)
        }
        ...
        function onProductEdit(e) {
            console.log("onProductEdit: ", e.detail);
        }

        // invalid Product added callback
        function onInvalidProduct(e) {
            console.log("onInvalidProduct: ", e.detail);
        }

        // invalid Product added callback
        function onProductClick(e) {
            console.log(e.detail);
            console.log("onProductClick: ", e.detail);
        }
    ...
    var demo2 = function() {
        var input = document.getElementById('kt_Productify_2');
        var Productify = new Productify(input, {
            ...
            callbacks: {
                add: console.log, // callback when adding a Product
                remove: console.log // callback when removing a Product
            }
        });
    }

    var demo3 = function() {
        var input = document.getElementById('kt_Productify_3');

        // init Productify script on the above inputs
        var Productify = new Productify(input);

        // add a class to Productify's input element
        //Productify.DOM.input.classList.remove('Productify__input');
        Productify.DOM.input.classList.add('form-control');
        Productify.DOM.input.setAttribute('placeholder', 'enter Product...');

        // re-place Productify's input element outside of the  element (Productify.DOM.scope), just before it
        Productify.DOM.scope.parentNode.insertBefore(Productify.DOM.input, Productify.DOM.scope);
    }

    var demo4 = function() {
        var input = document.getElementById('kt_Productify_4');
        var Productify = new Productify(input, {
            pattern: /^.{0,20}$/, // Validate typed Product(s) by Regex. Here maximum chars length is defined as "20"
        ...
        function transformProduct(ProductData) {
            ...
            ProductData.class = 'Productify__Product Productify__Product--' + states[KTUtil.getRandomInt(0, 7)];

            if (ProductData.value.toLowerCase() == 'shit') {
                ProductData.value = 's✲✲t'
            }
        }

        Productify.on('add', function(e) {
            console.log(e.detail)
        });

        Productify.on('invalid', function(e) {
    ...
    var demo5 = function() {
        // Init autocompletes
        var toEl = document.getElementById('kt_Productify_5');
        var ProductifyTo = new Productify(toEl, {
            ...
            templates: {
                dropdownItem : function(ProductData){
                    try{
                        return '<div class="Productify__dropdown__item">' +
                            '<div class="kt-media-card">' +
                            '    <span class="kt-media kt-media--'+(ProductData.initialsState?ProductData.initialsState:'')+'" style="background-image: url('+ProductData.pic+')">' +
                            '        <span>'+ProductData.initials+'</span>' +
                            '    </span>' +
                            '    <div class="kt-media-card__info">' +
                            '        <a href="#" class="kt-media-card__title">'+ProductData.value+'</a>' +
                            '        <span class="kt-media-card__desc">'+ProductData.email+'</span>' +
                            '    </div>' +
                            '</div>' +
                            '</div>';
                    }
